                ...
                <View style={styles.routineLeft}>
                  ...
                  <View style={styles.catDots}>
                    {cats.map((cat: string, i: number) => (
                      <View key={i} style={[styles.catDot, { backgroundColor: CATEGORY_COLORS[cat] ?? Colors.accent }]} />
                    ))}
                  </View>
                  <View>
...
const styles = StyleSheet.create({
  ...
  scroll: { padding: 24, paddingTop: 8, paddingBottom: 100 },
  ...
  createBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: Colors.accent, borderRadius: 12, paddingVertical: 12, paddingHorizontal: 24, marginTop: 8 },
  createBtnText: { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  routineCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: Colors.backgroundCard, borderRadius: 14, padding: 16, marginBottom: 12 },
  routineLeft: { flexDirection: 'row', alignItems: 'center', gap: 14, flex: 1 },
  catDots: { flexDirection: 'column', gap: 4 },
  catDot: { width: 8, height: 8, borderRadius: 4 },
  routineName: { fontSize: 16, fontWeight: '700', color: Colors.textPrimary, marginBottom: 3 },
