-- The convenience-adjacency reason strings in lib/makeup.ts said "Right
-- before/after her other lesson that day" — the club wants the actual
-- lesson name shown instead ("Right before Advanced Group Training"), and
-- no pronouns anywhere in the copy. `playerSchedule` entries didn't carry a
-- name at all before this, only day/time — adding one here.
-- Full body copied verbatim from 20260803220000_makeup_player_schedule_convenience.sql
-- (the current authoritative version) with one addition: a `label` field on
-- each `playerSchedule` entry.
