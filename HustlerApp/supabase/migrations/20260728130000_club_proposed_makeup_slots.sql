-- Mirror of the player-initiated makeup-slot request (20260728120000): here
-- the CLUB/coach picks a suggested slot and sends it to the player instead
-- of waiting for the player to find one themselves. Since the club already
-- vetted the slot, the player's response just confirms or declines it — no
-- second approval round-trip needed once they confirm.
--
-- 'owed' -> 'proposed'          (club sends a slot)         [club/coach]
-- 'proposed' -> 'scheduled'     (player confirms it)        [player]
-- 'proposed' -> 'owed'          (player declines it, or the
--                                 club withdraws it)          [player or club]
--
-- Reuses the exact same scheduled_date/start/end/court columns as the
-- player-initiated path — the two flows only ever differ in status and who's
-- allowed to move it, never in storage shape.
...
alter table public.makeup_lesson_credits add constraint makeup_lesson_credits_status_check
  check (status in ('owed', 'pending_approval', 'proposed', 'scheduled', 'done'));

-- The club/coach's existing "owner or own coach updates makeup credits"
-- policy already allows owed -> proposed (and a withdraw back to owed) with
-- no further change needed — that policy has no status restriction at all.

drop policy if exists "player responds to proposed makeup slot" on public.makeup_lesson_credits;
create policy "player responds to proposed makeup slot" on public.makeup_lesson_credits
  for update using (player_id = auth.uid() and status = 'proposed')
  with check (player_id = auth.uid() and status in ('scheduled', 'owed'));

-- ── Broaden get_makeup_scheduling_context: club/coach needs the same
-- candidate-slot data to suggest a time to the player, not just the player
-- fetching it for themselves. ─────────────────────────────────────────────
