-- A parallel migration (20260804130000_coach_shifts_and_club_hours.sql)
-- added `coachShifts` to get_makeup_scheduling_context, but its "copied
-- verbatim" comment pointed at 20260803200000_coach_time_off.sql — a base
-- that predates three later same-day changes: the `coaches` list scoped to
-- coaches who actually work with this player (20260803210000), the
-- `playerSchedule` array used for convenience ranking (20260803220000), and
-- its per-entry `label` (20260803240000). Copying from the stale base
-- silently reverted all three — confirmed live via an impersonated RPC call
-- that came back with `playerSchedule: null` and every active club coach
-- back in `coaches` regardless of relevance. That's why every makeup
-- suggestion was falling back to "same time as the missed lesson" instead
-- of ranking by adjacency to the player's other lessons — the data the
-- ranking depends on wasn't there anymore.
-- This migration merges both lines of change: coachShifts kept, coaches
-- relevance-filter and playerSchedule (with label) restored.
    ...
    'playerSchedule', coalesce((
      select jsonb_agg(x) from (
        select jsonb_build_object(
          ...
          'label', 'Private lesson with ' || coalesce(p4.full_name, 'coach')
        ) as x
        from public.schedule_assignments sa4
        join public.profiles p4 on p4.id = sa4.coach_id
