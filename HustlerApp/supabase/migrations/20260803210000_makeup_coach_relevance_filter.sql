-- get_makeup_scheduling_context's `coaches` list previously fed the
-- fallback-coach search with literally every active coach at the club —
-- so a missed private lesson could suggest a makeup with a coach the
-- player has never worked with. Per the club: the fallback pool should
-- only be coaches who already teach this player, either through a group
-- tier she's enrolled in or another private lesson — same relevance a
-- human scheduler would use. The original coach is unaffected (he's
-- always in this set too, via his own schedule_assignments row with her)
-- and is still tried first client-side in lib/makeup.ts.
-- Full body copied verbatim from 20260803200000_coach_time_off.sql (the
-- current authoritative version) with only the `coaches` field's subquery
-- changed.
